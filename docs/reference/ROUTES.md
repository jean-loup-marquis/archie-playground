...
| `/insights/:tab`     | `insights/shell.js`    | **Insights** — hub à 4 onglets (`usage` \| `performance` \| `voice` \| `team`) au-dessus des Playbooks. Flag `rootAnalytics`. `usage` est l'onglet par défaut ; `voice` et `team` sont des placeholders. Le shell rend le header + la nav d'onglets, chaque onglet possède son propre module (`insights/performance.js`, `insights/usage.js`). |
