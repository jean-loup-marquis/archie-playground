...
} from "../posts-store.js?v=88";
import { renderPostCard } from "./post-card.js?v=130";
import { renderTopPostEcho } from "./top-post-card.js?v=156";
import { renderClipCard } from "./clip-card.js?v=74";
import { onFeedbackClick } from "./feedback-control.js?v=17";
// Shared compact idea card — same component the standalone Ideas page uses.
import { renderCompactIdeaCard } from "./idea-card-compact.js?v=15";
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=112";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=476";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=121";
import { open as openRenameModal } from "./rename-modal.js?v=15";
import { getConnectedConnectors } from "../connectors-store.js?v=80";
import { askConnector } from "../connector-ask.js?v=60";
import { renderConnectorLogo } from "../connectors-view.js?v=62";
import { open as openConnectorsModal } from "./connectors-modal.js?v=64";
import { addMention as addComposerMention } from "../composer-mentions.js?v=86";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=114";
import { open as openImageStudioModal } from "./image-studio/index.js?v=130";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=130";
import { open as openConfirmModal } from "./confirm-modal.js?v=35";
...
export function init() {
  ...
  el.addEventListener("click", (event) => {
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=88").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=741").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
    ...
    if (reanalyzeBtn) {
      ...
      import("./toast.js?v=43").then(({ showToast }) =>
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=56").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=61"), import("../folders-store.js?v=56")]).then(
...
function onPostImageRemove(postId) {
  ...
  renderPanel();
  import("./toast.js?v=43").then(({ showToast }) => showToast("Image removed"));
}
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=741").then(({ askAngleQuestion }) => {
