...
import { ideas as MOCK_IDEAS } from "../mocks.js?v=118";
import { isNewUser } from "../user-mode.js?v=36";
import { getPath } from "../router.js?v=54";
import { parseHashParams, setHashQuery } from "../url-state.js?v=35";
import { LANGUAGE_OPTIONS } from "../languages.js?v=15";
import {
  getPosts,
  removePost,
  insertPost,
...
} from "../posts-store.js?v=89";
import { renderPostCard } from "./post-card.js?v=131";
import { renderTopPostEcho } from "./top-post-card.js?v=157";
import { renderClipCard } from "./clip-card.js?v=75";
import { onFeedbackClick } from "./feedback-control.js?v=18";
// Shared compact idea card — same component the standalone Ideas page uses.
import { renderCompactIdeaCard } from "./idea-card-compact.js?v=16";
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=113";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=477";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=122";
import { open as openRenameModal } from "./rename-modal.js?v=16";
import { getConnectedConnectors } from "../connectors-store.js?v=81";
import { askConnector } from "../connector-ask.js?v=61";
import { renderConnectorLogo } from "../connectors-view.js?v=63";
import { open as openConnectorsModal } from "./connectors-modal.js?v=65";
import { addMention as addComposerMention } from "../composer-mentions.js?v=87";
import { iconFor } from "../file-kinds.js?v=34";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=115";
import { open as openImageStudioModal } from "./image-studio/index.js?v=131";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=131";
import { open as openConfirmModal } from "./confirm-modal.js?v=36";
...
export function init() {
  ...
  el.addEventListener("click", (event) => {
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=89").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=742").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
    ...
    if (reanalyzeBtn) {
      ...
      import("./toast.js?v=44").then(({ showToast }) =>
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=57").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=62"), import("../folders-store.js?v=57")]).then(
...
function onPostImageRemove(postId) {
  ...
  renderPanel();
  import("./toast.js?v=44").then(({ showToast }) => showToast("Image removed"));
}
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=742").then(({ askAngleQuestion }) => {
