...
import { FORMATS, NETWORK_FORMATS, CLIP_RATIO_ORDER, ratioNetworksMeta, ratioValue } from "../clip-formats.js?v=61";
import { iconFor } from "../file-kinds.js?v=30";
import { DEFAULT_PRESET, buildCaptions, videoForClip } from "../clip-captions.js?v=15";
...
function renderOptions() {
  ...
  if (editorTab === "subtitles") {
    import("../caption-editor.js?v=28").then(({ refreshControls }) => refreshControls());
  }
}
...
function syncCaptionMount() {
  ...
  import("../caption-editor.js?v=28").then((mod) => {
    capMod = mod;
    if (want) {
...
function close() {
  ...
  if (captionMounted) {
    import("../caption-editor.js?v=28").then(({ unmount }) => unmount());
    captionMounted = false;
  }
