...
import { FORMATS, NETWORK_FORMATS, CLIP_RATIO_ORDER, ratioNetworksMeta, ratioValue } from "../clip-formats.js?v=58";
import { iconFor } from "../file-kinds.js?v=27";
import { DEFAULT_PRESET, buildCaptions, videoForClip } from "../clip-captions.js?v=12";
...
function renderOptions() {
  ...
  if (editorTab === "subtitles") {
    import("../caption-editor.js?v=25").then(({ refreshControls }) => refreshControls());
  }
}
...
function syncCaptionMount() {
  ...
  import("../caption-editor.js?v=25").then((mod) => {
    capMod = mod;
    if (want) {
...
function close() {
  ...
  if (captionMounted) {
    import("../caption-editor.js?v=25").then(({ unmount }) => unmount());
    captionMounted = false;
  }
