...
import { FORMATS, NETWORK_FORMATS, CLIP_RATIO_ORDER, ratioNetworksMeta, ratioValue } from "../clip-formats.js?v=60";
import { iconFor } from "../file-kinds.js?v=29";
import { DEFAULT_PRESET, buildCaptions, videoForClip } from "../clip-captions.js?v=14";
...
function renderOptions() {
  ...
  if (editorTab === "subtitles") {
    import("../caption-editor.js?v=27").then(({ refreshControls }) => refreshControls());
  }
}
...
function syncCaptionMount() {
  ...
  import("../caption-editor.js?v=27").then((mod) => {
    capMod = mod;
    if (want) {
...
function onModalClick(event) {
  ...
  if (action === "delete-clip") {
    ...
    const id = editingId;
    import("./confirm-modal.js?v=31").then(({ open }) => {
      open({
...
function close() {
  ...
  if (captionMounted) {
    import("../caption-editor.js?v=27").then(({ unmount }) => unmount());
    captionMounted = false;
  }
