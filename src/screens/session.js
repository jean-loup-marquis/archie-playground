import { html, raw, escapeHtml, escapeAttr as escapeHtmlAttr } from "../utils.js?v=44";
import { navigate } from "../router.js?v=53";
import { renderTopbar } from "../components/topbar.js?v=509";
import { socialAccounts, chatStarters, connectorDocs } from "../mocks.js?v=110";
import {
...
} from "../social-profiles.js?v=84";
import { FORMATS, formatsForNetwork, defaultFormatFor, clipFormatItems } from "../clip-formats.js?v=64";
import { CLIP_SUBTITLE_ITEMS, CLIP_SUBTITLE_LABEL } from "../clip-subtitles.js?v=14";
import { getSessionById, getSessions, subscribe as subscribeSessions } from "../sessions-store.js?v=59";
...
import { renderSidebar } from "../components/sidebar.js?v=476";
import { getContextById, getContexts, getDefaultContext, updateContext } from "../contexts-store.js?v=96";
...
} from "../assistant.js?v=118";
import { iconFor as fileIconForKind } from "../file-kinds.js?v=33";
import { getSources, getIdeas, extractVideoIdeas } from "../library.js?v=112";
import { wireLibraryActions, renderSourcesBulkBar, renderIdeasBulkBar } from "../library-actions.js?v=101";
import {
...
} from "../composer-mentions.js?v=86";
import { getPosts, addPostDraft, setSubtitleStyle, subscribe as subscribePostsStore } from "../posts-store.js?v=88";
import { startDraftFlow, executeDraft, executeDraftBatch, getAnglesForIdea } from "../draft-flow.js?v=116";
import { startActionPickerFlow, handleActionPick } from "../start-flow.js?v=98";
import * as topPostsFlow from "../top-posts-flow.js?v=139";
import {
  renderTopPostsBoard,
  renderTopPostEcho,
  renderTopPostsWidget,
  TOP_POSTS_LIMIT,
} from "../components/top-post-card.js?v=156";
import { getTopPost } from "../top-posts-store.js?v=111";
import { renderEmptyState } from "../components/empty-state.js?v=25";
import * as sidebarWizard from "../sidebar-wizard.js?v=104";
import * as inlineQuestion from "../inline-question.js?v=61";
import * as clipStudio from "../clip-studio.js?v=82";
import * as batchStudio from "../batch-studio.js?v=17";
import { askConnector } from "../connector-ask.js?v=60";
import { getConnectedConnectors, findConnector, setConnectorStatus } from "../connectors-store.js?v=80";
import { renderConnectorLogo } from "../connectors-view.js?v=62";
import {
  getActiveConnector,
  clearActiveConnector,
  subscribe as subscribeComposerConnector,
} from "../composer-connector.js?v=15";
import { isFlagOn } from "../feature-flags.js?v=43";
import { pillarForBrief, getPillarsForPlaybook, subscribe as subscribePillars } from "../pillars-store.js?v=29";
import { getActivePlaybook, getActivePlaybookId } from "../active-playbook.js?v=104";
...
import { BRIEF_CHAT_HANDOFF, attachBriefToChat, openBriefInChat } from "../brief-flow.js?v=61";
import { PILLAR_CHAT_HANDOFF, attachPillarToChat } from "../pillar-flow.js?v=36";
import { open as openPillarModal } from "../components/pillar-modal.js?v=96";
import { getLaneById, getLanes } from "../research-store.js?v=71";
import * as contextBuilder from "../context-builder.js?v=476";
import { renderPicker } from "./_analyse-common.js?v=68";
import { renderSourceCard } from "../components/source-card.js?v=46";
import { renderIdeaCard } from "../components/idea-card.js?v=40";
import { renderCompactIdeaCard } from "../components/idea-card-compact.js?v=15";
import {
  contentState,
  renderContentWorkspace as renderSharedContentWorkspace,
  rerenderContentWorkspaceBody,
  renderContentEmptyState,
} from "../components/content-workspace.js?v=38";
import { open as openVideoClipsModal } from "../components/video-clips-modal.js?v=112";
import { open as openChatPickerModal } from "../components/chat-picker-modal.js?v=120";
import { open as openAddSourceModal } from "../components/add-source-modal.js?v=121";
import { open as openConnectorsModal } from "../components/connectors-modal.js?v=64";
import { dropzoneHTML } from "../components/dropzone.js?v=14";
import {
  classifyFile,
  startFileUpload,
  startUrlImport,
  ...
  setSourceIdeaCount,
  addReadySource,
} from "../sources-stream.js?v=110";
import { renderClipCard } from "../components/clip-card.js?v=74";
import { onFeedbackClick } from "../components/feedback-control.js?v=17";
...
import { openIdeaArticle, openIdeaPicker, openPillarPicker } from "../components/research-modals.js?v=196";
import { findResearchSource } from "../research-catalog.js?v=39";
import { objectiveCardsFor } from "../mocks.js?v=110";
import { objectiveTier, TIER_LABELS, TIER_STATUS_CLASS, TIER_ORDER } from "../objective-scoring.js?v=24";
import {
  isOpen as isObjectiveAlertOpen,
  arm as armObjectiveAlert,
  commitArmed as commitObjectiveAlert,
  mutedUntilLabel,
} from "../objective-alerts-store.js?v=5";
...
import { briefBasis } from "../components/brief-card.js?v=95";
import {
...
} from "../components/right-panel.js?v=643";
import { setHandoff, consumeHandoff, hasHandoff } from "../handoff.js?v=33";
import { startTopicChat, TOPIC_CHAT_HANDOFF } from "../topic-flow.js?v=58";
import { parseHashParams, setHashQuery } from "../url-state.js?v=34";
import { updateLoadingWatchdog, stopThinkingTimer } from "./session/thinking-chip.js?v=76";
import { startIntakeLifecycle } from "./session/intake-lifecycle.js?v=84";
import { rebindWizardKeyboard } from "./session/wizard-keyboard.js?v=100";
...
import {
  ...
  renderResultCard,
} from "./session/thread-turns.js?v=14";
...
function renderAssistantPanel(session, attachedContext) {
  ...
  return html`
    <aside class="session__assistant" aria-label="Assistant panel">
      ...
      >
        ${isEmptyConversation
          ? raw(renderEmptyHero(session.id, composerMarkup, attachedContext))
...
function renderStarterTopicCard(brief) {
  ...
  const pillarLabel = pillar
    ...
    : "";
  // THE SOURCE JOINS THE CRUMB, for one source only — the internal one, which is
  // the only source whose topics are claims about the reader's own work rather than
  // observations about the market. Naming it is what stops "your video does 3× the
  // reach" reading like something a competitor did.
  //
  // This partly reverses the note above: the second segment was dropped because
  // every row came from the same feed, so it repeated. That stopped being true the
  // day a lane could hold two sources — but it is still true of the market ones, so
  // they stay bare and the crumb keeps two segments at most.
  const srcLabel =
    brief.basis && src
      ? `<span class="starter-topic__src" title="From your own published posts"><i class="${escapeHtml(src.icon)}" aria-hidden="true"></i><span>${escapeHtml(src.name)}</span></span>`
      : "";
  const crumb =
    pbLabel || srcLabel || pillarLabel
      ? `<span class="starter-topic__crumb">${pbLabel}${srcLabel}${pillarLabel}</span>`
      : "";
  ...
  return `
    ...
    >
      ...
      <!-- The basis, in the feed card's own class for the same reason the summary
           above uses the feed's: one object, one treatment. A measured claim states
           what it counted wherever it is read — and this row is where it is read
           FIRST, on the way into the chat that would act on it. -->
      ${
        brief.basis
          ? `<span class="topics-card__basis"><i class="ap-icon-information-circle" aria-hidden="true"></i><span>${escapeHtml(briefBasis(brief))}</span></span>`
          : ""
      }
...
//
// The third thing this screen can put in front of someone, and the one that is
// neither a topic nor a workflow: a declared objective is slipping. It is an
// ALERT, which is exactly why it is here rather than in the Topic Feed or under a
// pillar — it names no subject to write about and files under nothing. It says the
// plan is off, which is a fact about the account rather than a proposal, and the
// only place a fact like that is worth stating is where the work starts.
//
// SAME CHROME, OWN IDENTITY. Same card, same label treatment, and the same
// prompt-injection behaviour as a workflow starter (`data-starter-prompt` — the
// existing handler fills the composer and leaves the cursor at the end, so there is
// no new plumbing and the sentence is editable before it is sent). What is its own
// is the VERDICT PILL: nothing else on this screen carries one, and it is the app's
// established way of saying how far off something is.
//
...
// this render is still the PREVIOUS chat's — so a chat opened on Customer stories
// showed Acme's numbers. The topic list below can live with that (a store notify
// repaints it a frame later); an alert stating a brand's figures cannot.
function offTrackObjectives(attachedContext = null) {
  if (!isFlagOn("objectiveAlerts")) return [];
  const context = attachedContext || getActivePlaybook();
  if (!context) return [];
  return (
    objectiveCardsFor(context)
      .map((o) => ({
        ...o,
      ...
      // Only the ones still asking. An objective the reader has answered is muted
      // until the next read and one they set aside is out for this cycle — both keep
      // their verdict and both are still in Insights, which is where a state is read
      // rather than acted on.
      .filter((o) => isObjectiveAlertOpen(o.playbookId, o.objective))
      .sort((a, b) => TIER_ORDER[a.tier] - TIER_ORDER[b.tier] || a.progress - b.progress)
  );
}

// The gap in words, from the numbers the objective already carries — nothing is
// recomputed here. The trend comes second but decides the sentence: a target you
// are short of and climbing towards is not the same news as the same number
// sliding.
function objectiveGapLine(o) {
  const trend =
    o.variationPercent < 0
      ? `down ${Math.abs(o.variationPercent)}% in 30 days`
      : o.variationPercent === 0
        ? "flat for 30 days"
        : `up ${o.variationPercent}% in 30 days`;
  return `${o.value} of ${o.goal} ${o.metric} · ${trend}`;
}

function renderObjectiveAlertCard(o) {
  const pill = `<span class="ap-status ${TIER_STATUS_CLASS[o.tier]} no-dot">${TIER_LABELS[o.tier]}</span>`;
  // The prompt IS the action: it states the gap and asks the question the reader
  // would have typed. A button that fired something would decide for them; this
  // lands in the composer where they can change it first.
  const prompt = `${o.objective} is at ${o.value} of ${o.goal} ${o.metric} and ${
    o.variationPercent < 0 ? `down ${Math.abs(o.variationPercent)}%` : "not moving"
  } this month. What should I post to close the gap?`;
  return `
    <button
      type="button"
      class="starter-card starter-card--objective"
      data-starter="objective-${escapeHtml(o.objective)}"
      data-starter-prompt="${escapeHtml(prompt)}"
      data-objective-alert="${escapeHtml(o.objective)}"
      data-objective-playbook="${escapeHtml(o.playbookId || "")}"
    >
      <span class="starter-topic__head">
        <span class="starter-topic__crumb">
          <span class="starter-topic__pb" title="From ${escapeHtml(o.playbookName)}"
            ><i class="ap-icon-target" aria-hidden="true"></i><span>${escapeHtml(o.playbookName)}</span></span
          >
        </span>
        ${pill}
      </span>
      <span class="starter-card__title">${escapeHtml(o.objective)}</span>
      <span class="objective-alert__gap">${escapeHtml(objectiveGapLine(o))}</span>
      <span class="starter-card__cta ap-link standalone small"
        >Ask what to post about it<i class="ap-icon-arrow-right" aria-hidden="true"></i
      ></span>
    </button>
  `;
}
...
// the header there.
function renderEmptyHero(sessionId, composerMarkup = "", attachedContext = null) {
  ...
  return html`
    <div class="empty-chat" data-empty-chat>
      ...
      <!-- The alert comes FIRST of the three blocks, and only when there is one. A
           slipping objective is not a proposal to read, it is the frame the two
...
function bindSession(root, session) {
  ...
  function submitInput() {
    ...
    input.value = "";
    // Sending IS acting on an off-track objective, and the only thing that is: the
    // reader took the sentence into the conversation. It mutes until the next read
    // — never "resolved", because the objective recovers when the numbers do, not
    // when a draft goes out. The toast says exactly that, so nothing on screen
    // claims a fix that has not happened.
    const acted = commitObjectiveAlert(session.id);
    if (acted) {
      showToast(`Set aside ${mutedUntilLabel(acted.playbookId)} — ${acted.objective} stays in Insights`);
    }
  ...
  root.addEventListener(
    ...
    (event) => {
      ...
      if (starterBtn && starterBtn.dataset.starterPrompt != null) {
        const input = getInput();
        if (!input) return;
        // An objective row ARMS the session rather than answering: filling the
        // composer is a proposal, and the reader can still read the sentence and
        // walk away. commitArmed below is what actually mutes it, on send.
        if (starterBtn.dataset.objectiveAlert) {
          armObjectiveAlert(session.id, starterBtn.dataset.objectivePlaybook, starterBtn.dataset.objectiveAlert);
        }
