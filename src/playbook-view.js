...
import { html, raw, escapeHtml as esc } from "./utils.js?v=45";
import { openPillarInChat } from "./pillar-flow.js?v=37";
import { analyzeWebsite, discoverCompetitors, competitorKey } from "./context-mock-analysis.js?v=39";
import { LANGUAGE_OPTIONS, emptyVoiceEntry } from "./languages.js?v=15";
...
export function mount(target, config) {
  ...
  if (cfg.openObjective) {
    ...
    const wanted = labels.find((l) => (l || "").toLowerCase() === cfg.openObjective.toLowerCase());
    if (wanted) openObjectiveEditorFor(wanted);
  }
