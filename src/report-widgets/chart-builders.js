import { colorByName, getDataColor, greyColor } from "./report-colors.js?v=24";
import { formatGroupedNumber } from "./number-formatting.js?v=24";
...
// The SPLINE chart type. Same axes as the column chart, deliberately: it is the same
// metric over the same thirty days, and only the reading changes — "show trends over
// time" against "compare values over time", which is how the two are labelled in the
// widget editor. Markers off, because thirty of them per series is noise.
export function splineOptions({ categories } = {}) {
  const base = defaultOptions();
  return {
    ...base,
    ...
    plotOptions: { ...base.plotOptions, spline: { lineWidth: 2, marker: { enabled: false } } },
  };
}

export function buildSplineChartSeries(seriesData) {
  return seriesData.map((metric, index) => ({
    type: "spline",
    name: metric.name,
    data: metric.data,
...
// The DISTRIBUTION chart type — "show proportional breakdown". It answers WHO rather
// than WHEN, so the day axis goes away entirely and each series collapses to its own
// total. A donut rather than a full pie, as the report's own distribution widget draws
// it, and the labels stay in the legend: thirteen leader lines around a 470px card is
// what the legend already exists to avoid.
export function distributionOptions() {
  ...
  return {
    ...
    xAxis: undefined,
    yAxis: undefined,
    tooltip: { ...base.tooltip, shared: false },
    plotOptions: {
      ...base.plotOptions,
      pie: {
        innerSize: "62%",
...
// One slice per series, summed over the range. An empty series is dropped rather than
// drawn as a zero-width slice — unlike a column chart there is no axis position to
// keep, so nothing is lost by its absence.
export function buildDistributionSeries(seriesData) {
  return [
    {
      type: "pie",
      name: "Share",
      data: seriesData
        .map((metric, index) => ({
          name: metric.name,
          y: metric.data.reduce((sum, v) => sum + v, 0),
          color: metric.color ? colorByName(metric.color, index) : getDataColor(index),
        }))
