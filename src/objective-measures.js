...
export function paceFor(measure) {
  ...
  if (pct >= MEASURE_TIER_FLOORS.strong) return { id: "ahead", label: pct >= 100 ? "AHEAD" : "ON PACE", tone: "green" };
  if (pct >= MEASURE_TIER_FLOORS.watch) return { id: "on-pace", label: "ON PACE", tone: "green" };
...
// and at least one off → At risk, anything else → Watch. An objective in
...
export function objectiveVerdict(resolved) {
  ...
  const tier = total === 0 ? null : onTrack === total ? "strong" : onTrack === 0 && hasOff ? "at-risk" : "watch";
  return {
    onTrack,
    total,
    tier,
...
// ── Per-profile breakdown (detail screen 6a) ────────────────────────────────
// Splits a measure's current reading across the profiles its scope covers.
// Same honesty rules as scopedBaseline: rates don't split; the jitter is
// deterministic (authored offsets, not randomness). Noba's clicks are authored
// to the design's own worked example (IG 74 −9% · FB 38 −4%).
const PROFILE_SPLITS = {
  "ctx-noba::clicks": [
    { network: "instagram", share: 0.66, trendPct: -9 },
    { network: "facebook", share: 0.34, trendPct: -4 },
  ],
};

const PROFILE_JITTER = [1.12, 0.88, 1.04, 0.96];

export function profileSplit(measure, contextId) {
  const profiles = getConnectedProfiles();
  const inScope = measure.scope?.network
    ? profiles.filter(
        (p) =>
          p.platform === measure.scope.network &&
          (!measure.scope.profileIds?.length || measure.scope.profileIds.includes(p.id)),
      )
    : profiles;
  const total = parseMetricValue(measure.baselineValue);
...
// The connected networks a measure's scope does NOT cover — the detail screen
// shows each as a dashed "isn't in this measure's scope" tile.
export function outOfScopeNetworks(measure) {
  if (!measure.scope?.network) return [];
  const seen = new Set();
  getConnectedProfiles().forEach((p) => seen.add(p.platform));
  seen.delete(measure.scope.network);
  return [...seen];
}

// ── The posts that carry a measure (detail screen 6a) ──────────────────────
// Authored evidence rows — buildTopPosts() generates laddered lorem with no
// clicks and no metric attachment, so this small table lives beside the
// baselines it illustrates. Defaults per metric, overrides per Playbook;
// Noba's clicks rows are the design's own.
const OBJECTIVE_POSTS = {
  clicks: [
    {
      network: "LinkedIn",
      date: "Jun 18",
...
const OBJECTIVE_POSTS_BY_CONTEXT = {
  "ctx-noba": {
    clicks: [
      {
        network: "Instagram",
...
export function measurePosts(metricId, contextId) {
  return (OBJECTIVE_POSTS_BY_CONTEXT[contextId] || {})[metricId] || OBJECTIVE_POSTS[metricId] || [];
}
